...
\usepackage{xurl}
\usepackage{hyperref}
\hypersetup{colorlinks=false,pdfborder={0 0 0},pdfborderstyle={/S/U/W 0}}
...
\author{Gratchia Mkrttchian\\\small Higher Colleges of Technology--CERT\\\small \texttt{gmkrttchian1@hct.ac.ae}}
\date{}
\begin{document}
\maketitle
...
\section{Common-stem rooted construction}
...
\item therefore the shifted retained holonomy-log coordinate system is both gauge-covariant in a common color frame and reflection-compatible at finite group level.
\end{enumerate}
...
\subsection{Finite path-list check}
...
\hline
rooted path-word SHA256 &
\begin{tabular}[t]{@{}l@{}}
\texttt{\scriptsize 1EB6E829D66DCDFA6E12E18CACA3C09}\\
\texttt{\scriptsize A3EDFC3EA56E7EBE0D7ED1E05CA01616C}
\end{tabular}
\\
\hline
\end{longtable}
...
\begin{center}
\small
\begin{longtable}{|>{\raggedright\arraybackslash}p{0.18\linewidth}|>{\raggedright\arraybackslash}p{0.38\linewidth}|>{\raggedright\arraybackslash}p{0.34\linewidth}|}
\hline
...
\section{Relation to possible multiscale use}
...
\begin{itemize}[leftmargin=*]
\item an exact single-shell constructive theorem;
\item a deterministic compact-log remainder theorem;
\item compatibility of the rooted construction with any later shell schedule;
...
\item exact nonlinear single-shell disintegration/Radon--Nikodym theorem;
...
\hline
\endhead
exact certificates & {\scriptsize\path{calculations/c5x_exact_finite_claim_certificates.py}} & {\scriptsize\path{outputs/c5x_exact_finite_claim_certificates.json}} \\
\hline
linear/reflection-density check & {\scriptsize\path{calculations/c5ax_reflection_retained_density.py}} & {\scriptsize\path{outputs/clean-start-c5ax-reflection-retained-density.md}} \\
\hline
shifted-coordinate check & {\scriptsize\path{calculations/c5ay_reflection_covariant_retained_coordinates.py}} & {\scriptsize\path{outputs/clean-start-c5ay-reflection-covariant-retained-coordinates.md}} \\
\hline
rooted-coordinate regression & {\scriptsize\path{calculations/c5az_shifted_coordinate_package_regression.py}} & {\scriptsize\path{outputs/clean-start-c5az-shifted-coordinate-package-regression.md}} \\
\hline
\end{longtable}
\end{center}
...
\hline
reproduction output folder & {\scriptsize\path{outputs/c5x-reproducibility-rerun/}} \\
\hline
SHA256 manifest & {\scriptsize\path{outputs/c5x-reproducibility-rerun/sha256_manifest.txt}} \\
\hline
...
\hline
downstream diagnostic assertions & passed with \texttt{-IncludeDownstream} \\
\hline
...
The manifest is not copied into the prose because it changes whenever a source file changes.  The authoritative records are \path{outputs/c5x-reproducibility-rerun/manifest.json} and \path{sha256_manifest.txt}; every listed file path is relative to the repository root.

The reproduction run verifies the intended finite-block pattern:

\begin{itemize}[leftmargin=*]
\item The linear quotient reflection implementation is roundoff-correct, while the original based finite-\(g\) retained log has a visible nonlinear reflection defect.
\item Uniform cyclic shifts \(1\) and \(5\) are exact; the paper uses shift \(1\), giving 12 same-order and 12 exact-inverse reflected paths with no unmatched shifted paths.
\item Rooted gauge covariance and reflection residuals are at roundoff scale, the rooted density is recomputed, and preservation of the shifted-core constants is explicitly false.
\end{itemize}
...
\hline
K. G. Wilson, *Confinement of quarks*, Phys. Rev. D 10, 2445--2459 (1974), DOI \texttt{10.1103/PhysRevD.10.2445}; see also Wilson's retrospective \texttt{arXiv:hep-lat/0412043} & Wilson lattice gauge variables and Wilson-loop action background & Wilson loops/traces are standard.  The paper's distinction is between trace/conjugacy variables and based Lie-algebra logs in one finite retained-coordinate system. \\
\hline
...
The finite-dimensional verification scripts, wrapper, manifest structure, and numerical-result explanations are available in the public reproducibility repository at \url{https://github.com/Gratchia/yang-mills-paper1-reproducibility}.
